...
| Admin | `/#/admin`, `admin@daivikpuja.in` / `admin123` (from `.env`) — full access |
...
Admin → Reports exports 19 reports as real `.xlsx` files (exceljs): customers, pandits, temples, pujas, bookings, payments, orders, kundalis, kundali payments, family members, customized puja requests, samagri, prasad, coupons, campaigns, payouts, revenue by month, puja performance, commission by month. Exports always contain **all records matching the filters** (status/date/mode/billing), reuse the same queries the admin tables show, exclude sensitive fields by design (password hashes, OTPs, tokens, KYC), require the admin role, and every download is written to an audit log (`export_logs`, viewable on the same tab). Service visibility toggles on the same tab hide Home/Online/Temple/Customized Puja, Kundali, Pandit, Temple, Prasad, Samagri or Astrology from the customer navigation instantly.
...
Other commands: `npm test` (API tests + engine tests), `npm run test:ui` (drives the real UI in jsdom), `npm run reset` (clear and reseed), `docker build -t daivikpooja . && docker run -p 3000:3000 -v dp:/data -e JWT_SECRET=... daivikpooja`.
