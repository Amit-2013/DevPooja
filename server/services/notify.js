...
function notify(userId, channel, message) {
  ...
  const job = channel === 'SMS' && u.mobile ? twilio(u.mobile, message, false)
    ...
    : channel === 'Email' && u.email ? sendgrid(u.email, 'DaivikPooja update', message) : null;
...
async function sendOtp(mobile, code) {
  ...
  try { await twilio(mobile, `Your DaivikPooja OTP is ${code}. Valid for 5 minutes.`, false); } catch (e) { console.error('[otp]', e.message); }
}
