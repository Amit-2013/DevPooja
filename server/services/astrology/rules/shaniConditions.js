...
function evaluate(view, now) {
  ...
  if (sat.dignity === 'Debilitated') { evidence.push('Saturn is debilitated in the birth chart.'); severity = severity || 'medium'; confidence = Math.max(confidence, 0.7); }
  if (sat.house === 8) { evidence.push('Saturn occupies the 8th house in the birth chart.'); severity = severity || 'low'; confidence = Math.max(confidence, 0.6); }

  if (!evidence.length) return null;
  return { detected: true, severity, confidence, evidence };
}
