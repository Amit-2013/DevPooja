...
const badge=s=>{if(lang==='hi'&&STATUS_HI[s])return'<span class="badge '+(s==='Completed'||s==='verified'||s==='Paid'||s==='Resolved'||s==='Processed'||s==='Delivered'?'ok':s==='New'||s==='pending'||s==='Packed'||s==='Dispatched'||s==='Scheduled'||s==='Sent'?'info':s==='Cancelled'||s==='rejected'?'bad':'warn')+'">'+STATUS_HI[s]+'</span>';
 const m={Completed:'ok',Confirmed:'info',Assigned:'info',Started:'warn',New:'warn',Cancelled:'bad',Open:'warn',Resolved:'ok',Paid:'ok',Pending:'warn',Delivered:'ok',Dispatched:'info',Packed:'info',Processed:'ok',Initiated:'warn',verified:'ok',pending:'warn',rejected:'bad',Sent:'ok',Scheduled:'info',PENDING:'warn',ON_HOLD:'bad',PROCESSING:'info',DISBURSED:'ok',FAILED:'bad',REVERSED:'warn'};return'<span class="badge '+(m[s]||'')+'">'+esc(s)+'</span>'};
...
function header(){
 ...
 $('#hdr').innerHTML='<div class="util"><div class="wrap"><span>'+(db&&db.config.demo?'Demo mode: sample pandits, temples and reviews. Test OTP is 123456.':'Verified pandits, samagri and prasad, booked online')+'</span><span>'+langSel+'<a href="#/about">'+(lang==='hi'?'हमारे बारे में':'About us')+'</a><a href="#/contact">'+(lang==='hi'?'संपर्क':'Contact')+'</a><a href="#/partner">'+(lang==='hi'?'साझेदारी':'Partner with us')+'</a><button data-act="theme" aria-label="Toggle light or dark theme">'+(lang==='hi'?'थीम':'Theme')+'</button></span></div></div><div class="nav"><div class="wrap navin"><a class="logo" href="#/">'+logoSvg+'<span>DaivikPooja<small>दैविक पूजा</small></span></a><nav class="links" id="links" aria-label="Main">'+navs.map(n=>'<a href="#/'+n[0]+'" class="'+(r.page===n[0]?'on':'')+'">'+t(n[1])+'</a>').join('')+'</nav><div class="navr"><button class="cartb" data-act="cart" aria-label="Cart">🛒'+(cart.length?'<i>'+cart.reduce((a,c)=>a+c.q,0)+'</i>':'')+'</button>'+usr+'<button class="burger" data-act="burger" aria-label="Menu">☰</button></div></div></div>';
} function footer(){$('#ftr').innerHTML='<div class="wrap"><div class="grid g4"><div><div class="logo" style="color:#fff">'+logoSvg+'<span>DaivikPooja</span></div><p class="sm mt"><b>DaivikPooja – Sanatan Seva Platform</b><br>The complete puja experience on one trusted platform: puja, verified pandit, samagri, prasad.</p></div><div><h4>Explore</h4><a href="#/pujas">Puja catalogue</a><a href="#/pandits">Verified pandits</a><a href="#/temples">Temples</a><a href="#/festivals">Festival calendar</a><a href="#/kundali">Kundali</a><a href="#/astrology">Astrology</a></div><div><h4>Shop and save</h4><a href="#/samagri">Samagri kits</a><a href="#/prasad">Prasad</a><a href="#/rewards">DaivikPooja Rewards</a><a href="#/plus">DaivikPooja Plus</a><a href="#/custom-puja">Customised puja</a><a href="#/corporate">Corporate puja</a></div><div><h4>Company</h4><a href="#/about">About</a><a href="#/contact">Contact and support</a><a href="#/partner">Pandit partners</a><a href="#/admin">Admin panel (demo)</a></div></div><p class="sm mt2" style="opacity:.7">'+(db&&db.config.demo?'Demo data is loaded. Prices, pandits, temples and reviews are illustrative.':'')+'</p></div>'}
