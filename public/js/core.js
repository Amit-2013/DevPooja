...
const badge=s=>{if(lang==='hi'&&STATUS_HI[s])return'<span class="badge '+(s==='Completed'||s==='verified'||s==='Paid'||s==='Resolved'||s==='Processed'||s==='Delivered'?'ok':s==='New'||s==='pending'||s==='Packed'||s==='Dispatched'||s==='Scheduled'||s==='Sent'?'info':s==='Cancelled'||s==='rejected'?'bad':'warn')+'">'+STATUS_HI[s]+'</span>';
 const m={Completed:'ok',Confirmed:'info',Assigned:'info',Started:'warn',New:'warn',Cancelled:'bad',Open:'warn',Resolved:'ok',Paid:'ok',Pending:'warn',Delivered:'ok',Dispatched:'info',Packed:'info',Processed:'ok',Initiated:'warn',verified:'ok',pending:'warn',rejected:'bad',Sent:'ok',Scheduled:'info'};return'<span class="badge '+(m[s]||'')+'">'+esc(s)+'</span>'};
