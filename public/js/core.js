...
function header(){
 ...
 $('#hdr').innerHTML='<div class="util"><div class="wrap"><span>'+(db&&db.config.demo?'Demo mode: sample pandits, temples and reviews. Test OTP is 123456.':'Verified pandits, samagri and prasad, booked online')+'</span><span><a href="#/about">About us</a><a href="#/contact">Contact</a><a href="#/partner">Partner with us</a><button data-act="lang">'+(lang==='en'?'हिन्दी':'English')+'</button><button data-act="theme" aria-label="Toggle light or dark theme">Theme</button></span></div></div><div class="nav"><div class="wrap navin"><a class="logo" href="#/">'+logoSvg+'<span>DeivikPooja<small>दैविक पूजा</small></span></a><nav class="links" id="links" aria-label="Main">'+navs.map(n=>'<a href="#/'+n[0]+'" class="'+(r.page===n[0]?'on':'')+'">'+t(n[1])+'</a>').join('')+'</nav><div class="navr"><button class="cartb" data-act="cart" aria-label="Cart">🛒'+(cart.length?'<i>'+cart.reduce((a,c)=>a+c.q,0)+'</i>':'')+'</button>'+usr+'<button class="burger" data-act="burger" aria-label="Menu">☰</button></div></div></div>';
}
function footer(){$('#ftr').innerHTML='<div class="wrap"><div class="grid g4"><div><div class="logo" style="color:#fff">'+logoSvg+'<span>DeivikPooja</span></div><p class="sm mt"><b>DeivikPooja – Sanatan Seva Platform</b><br>The complete puja experience on one trusted platform: puja, verified pandit, samagri, prasad.</p></div><div><h4>Explore</h4><a href="#/pujas">Puja catalogue</a><a href="#/pandits">Verified pandits</a><a href="#/temples">Temples</a><a href="#/festivals">Festival calendar</a><a href="#/astrology">Astrology</a></div><div><h4>Shop and save</h4><a href="#/samagri">Samagri kits</a><a href="#/prasad">Prasad</a><a href="#/rewards">DeivikPooja Rewards</a><a href="#/plus">DeivikPooja Plus</a><a href="#/corporate">Corporate puja</a></div><div><h4>Company</h4><a href="#/about">About</a><a href="#/contact">Contact and support</a><a href="#/partner">Pandit partners</a><a href="#/admin">Admin panel (demo)</a></div></div><p class="sm mt2" style="opacity:.7">'+(db&&db.config.demo?'Demo data is loaded. Prices, pandits, temples and reviews are illustrative.':'')+'</p></div>'}
