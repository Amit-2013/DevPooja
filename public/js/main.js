...
/* --- accounts, audit and puja photos (migration 009) --- */
 ...
 ammod(d){const patch=d.v==='primary'?{primary:1}:d.v==='publish'?{published:1}:d.v==='unpublish'?{published:0}:d.v==='approve'?{status:'APPROVED'}:d.v==='reject'?{status:'REJECTED'}:{status:d.v};api('/admin/media/'+d.id,{method:'PATCH',body:patch}).then(()=>{toast('Photo updated');amRefresh()}).catch(e=>toast(e.message))},
...
function drawMediaModal(){const all=PAGE.amData||[],t=PAGE.amtab||'all';const list=t==='all'?all:all.filter(m=>t==='seeded'||t==='pandit'?m.source===t:m.status===t);
 modal('<h2>Photos — '+esc((PU(PAGE.amediaPid)||{}).n||PAGE.amediaPid)+'</h2><div class="tabs mt">'+AMTABS.map(x=>'<button class="'+(t===x[0]?'on':'')+'" data-act="amtab" data-v="'+x[0]+'">'+x[1]+' ('+(x[0]==='all'?all.length:all.filter(m=>x[0]==='seeded'||x[0]==='pandit'?m.source===x[0]:m.status===x[0]).length)+')</button>').join('')+'<button class="" data-act="amcredits">Credits</button></div>'+
...
const pmCard=m=>'<div class="card flat" style="width:170px;padding:8px"><img src="'+mediaUrl(m.thumb||m.url)+'" alt="'+esc(m.altText||m.origName||'photo')+'" loading="lazy" style="width:100%;height:110px;object-fit:cover;border-radius:8px;background:#eee"><div class="sm mt">'+(m.status==='PENDING_ADMIN_REVIEW'?'<span class="badge">Pending review</span>':m.status==='APPROVED'?(m.isPublished?'<span class="badge ok">Live on site</span>':'<span class="badge info">Approved</span>'):'<span class="badge bad">Rejected</span>')+'</div><div class="sm mut">'+esc(m.altText||m.origName||'').slice(0,44)+'</div><div class="sm mut">'+(m.createdAt?new Date(m.createdAt).toLocaleDateString('en-IN'):'')+'</div>'+(m.status==='PENDING_ADMIN_REVIEW'?'<button class="btn s bad mt" data-act="pmdel" data-id="'+m.id+'">Delete</button>':'')+'</div>';
