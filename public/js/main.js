...
/* --- accounts, audit and puja photos (migration 009) --- */
 ...
 /* puja photo manager (admin + pandit) */
 async amedia(d){PAGE.amediaPid=d.id;try{const r=await api('/admin/pujas/'+d.id+'/media');modal('<h2>Photos — '+esc((PU(d.id)||{}).n||d.id)+'</h2><div class="note mb">Drag-free upload: choose one or more JPG/PNG/WEBP files. First upload becomes the primary photo. Pandit uploads arrive here as <b>Pending</b> and are not public until you approve and publish them.</div><label class="btn s blk mt" style="cursor:pointer">Upload photos<input type="file" id="amf" multiple accept="image/jpeg,image/png,image/webp" style="display:none" data-in="amfile"></label><div id="amgrid" class="row mt" style="flex-wrap:wrap">'+r.media.map(m=>amCard(m)).join('')+'</div><p class="sm mut mt">Click a photo to open it; use the buttons to moderate. Deleted files are removed from the server.</p>',true)}catch(e){toast(e.message)}},
 async amupload(){const inp=$('#amf');if(!inp||!inp.files||!inp.files.length)return toast('Choose photos first');const fd=new FormData();[...inp.files].forEach(f=>fd.append('media',f));  try{const r=await api('/admin/pujas/'+PAGE.amediaPid+'/media',{method:'POST',form:fd});$('#amgrid').innerHTML=r.media.map(amCard).join('');toast('Uploaded '+r.media.length+' photo(s)')}catch(e){toast(e.message)}},
 ammod(d){const patch=d.v==='primary'?{primary:1}:d.v==='publish'?{published:1}:d.v==='unpublish'?{published:0}:d.v==='approve'?{status:'APPROVED',published:1}:{status:'REJECTED',published:0};api('/admin/media/'+d.id,{method:'PATCH',body:patch}).then(()=>{toast('Photo updated');amRefresh()}).catch(e=>toast(e.message))},
 ...
 async pmupload(){const inp=$('#pmf'),sel=$('#pmb');if(!inp||!inp.files||!inp.files.length)return toast('Choose photos first');if(!sel||!sel.value)return toast('Select the booking first');const fd=new FormData();[...inp.files].forEach(f=>fd.append('media',f));fd.append('bookingId',sel.value);
...
/* media manager helpers (admin photo gallery modal) */
const amCard=m=>'<div class="card flat" style="width:150px;padding:8px"><img src="'+mediaUrl(m.url)+'" alt="'+esc(m.origName||'Puja photo')+'" loading="lazy" style="width:100%;height:100px;object-fit:cover;border-radius:8px;background:#eee"><div class="sm mt">'+(m.isPrimary?'<span class="badge ok">Primary</span> ':'')+(m.isPublished?'<span class="badge info">Published</span>':m.status==='PENDING_ADMIN_REVIEW'?'<span class="badge">Pending</span>':'<span class="badge bad">Rejected</span>')+'</div><div class="sm mut">'+esc((m.origName||'').slice(0,18))+'</div><div class="row mt" style="gap:4px;flex-wrap:wrap">'+(m.status!=='APPROVED'?'<button class="btn s" data-act="ammod" data-id="'+m.id+'" data-v="approve">Approve</button>':'<button class="btn s ghost" data-act="ammod" data-id="'+m.id+'" data-v="'+(m.isPublished?'unpublish':'publish')+'">'+(m.isPublished?'Unpublish':'Publish')+'</button>')+'<button class="btn s ghost" data-act="ammod" data-id="'+m.id+'" data-v="primary">Primary</button><a class="btn s ghost" href="'+mediaUrl('/api/media/'+m.id+'/download')+'" download>Get</a><button class="btn s bad" data-act="amdel" data-id="'+m.id+'">Del</button></div></div>';
function amRefresh(){if(!PAGE.amediaPid)return;api('/admin/pujas/'+PAGE.amediaPid+'/media').then(r=>{const g=$('#amgrid');if(g)g.innerHTML=r.media.map(amCard).join('')}).catch(()=>{})}
/* pandit photo card + actions */
const pmCard=m=>'<div class="card flat" style="width:160px;padding:8px"><img src="'+mediaUrl(m.url)+'" alt="'+esc(m.origName||'photo')+'" loading="lazy" style="width:100%;height:100px;object-fit:cover;border-radius:8px;background:#eee"><div class="sm mt">'+(m.status==='PENDING_ADMIN_REVIEW'?'<span class="badge">Pending review</span>':m.status==='APPROVED'?(m.isPublished?'<span class="badge ok">Live</span>':'<span class="badge info">Approved</span>'):'<span class="badge bad">Rejected</span>')+'</div><div class="sm mut">'+esc((m.origName||'').slice(0,16))+'</div>'+(m.status==='PENDING_ADMIN_REVIEW'?'<button class="btn s bad mt" data-act="pmdel" data-id="'+m.id+'">Delete</button>':'')+'</div>';
